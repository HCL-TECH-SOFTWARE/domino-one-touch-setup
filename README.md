...
The Domino container image and the Domino on Linux start script support JSON templating.
A JSON file with environment variable names can be used to setup a server.
...
The setup uses the same environment variables used for an environment variable setup.
